...
func (e *Engine) record(res Result) {
	...
	// Per-host port-scan progress for the live recon / scan view.
	e.state.recordHost(res.Addr.String(), res.Port, res.Response)
	// dials/hour
	...
	case RespBusy, RespVoice:
		e.logTarget(res, res.Response.Tag())
	case RespNoDialtone:
		e.logTarget(res, fmt.Sprintf("Unreachable #%d", res.Tries))
	case RespRingout:
		e.logTarget(res, fmt.Sprintf("No-reply (%d)", res.Rings))
	case RespTimeout:
...
// socketResult renders a probe verdict as a terse socket/network message for
// the Socket window.
...
func (e *Engine) banner() {
	e.logf("DARKCIDR started on %s", time.Now().Format("02-Jan-06"))
