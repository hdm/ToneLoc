...
// ServeGame serves DARKCIDR in the browser. There is exactly one game
// implementation -- the native terminal game (internal/game) -- and the browser
// runs it through the same ghostty.js terminal the scanner uses: every
// WebSocket client gets a fresh run, streamed as the very ANSI/VT output a local
// terminal would draw, with keystrokes flowing back over the socket.
func ServeGame(opts Options, seed *game.Seed) error {
	mux := http.NewServeMux()
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/" {
			http.NotFound(w, r)
...
// serveGameSession runs one terminal DARKCIDR session for a browser client,
// seeded uniquely per connection. The fixed 80x25 screen means there is no
// resize negotiation: the page sizes its terminal to 80x25 itself.
func serveGameSession(conn *websocket.Conn, seed *game.Seed) {
	defer conn.Close()
	...
	seedNum := uint32(time.Now().UnixNano())
	if err := game.Run(ctx, out, keys, seed, seedNum, game.RunOptions{}); err != nil && err != io.EOF {
		log.Printf("game session ended: %v", err)
	}
}
...
func listen(opts Options, mux http.Handler, label string) error {
	...
	case opts.TLS && opts.Domain != "":
		...
		fmt.Printf("DARKCIDR %s -- %s://%s/  (ACME cert for %s; needs :80 and :443 reachable)\n",
	...
	default:
		...
		fmt.Printf("DARKCIDR %s -- open http://%s/ in your browser\n", label, friendly(addr))
