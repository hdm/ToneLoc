...
DARKCIDR imports [`zmap-go`](https://github.com/hdm/zmap-go) as a normal Go
module, so a single command installs it:

```sh
go install github.com/hdm/toneloc/cmd/darkcidr@latest
# or, from a clone:  go build -o darkcidr ./cmd/darkcidr   (or: make)
```
...
**With no arguments**, DARKCIDR auto-detects every local network this machine is
attached to and sweeps them with a real TCP connect scan, hitting the most
common ports first (80, 443, 22, 135, 445, 3389, ...):

```sh
darkcidr            # scan all local networks, common ports first
```
...
```sh
darkcidr 192.168.1.X /p:22,80,443        # dial .0-.255 on three ports
darkcidr 10.0.X.X --connect --wait 2s    # a /16, real TCP connect scan
darkcidr 198.51.100.0/24 --zmap          # real zmap SYN scan (needs root)
darkcidr CORP /M:172.16.0.X /R:1-50      # bound the last octet to .1-.50
darkcidr 203.0.113.X --web :9000         # serve the UI in a browser
```
...
Open TCP ports are only the start. DARKCIDR runs a four-tool pipeline:
...
nerva and brutus are **compiled in as Go libraries** — DARKCIDR is a single
...
Every scan writes a resumable **session log** (`darkcidr-<id>.session`) capturing
...
```sh
darkcidr 203.0.113.X --web --tls --domain scan.example.com
```

Or bring your own certificate instead of ACME:

```sh
darkcidr 203.0.113.X --web --tls-cert fullchain.pem --tls-key privkey.pem
```

Both work for the game too (`darkcidr --game :8090 --tls --domain play.example.com`).
...
DARKCIDR's game mode is a war-dialing roguelike, and there is **exactly one
implementation**: a native Go terminal game (`internal/game`) drawn with the
same DOS renderer the scanner uses. In the terminal it fills the whole window
(and follows resizes); the layout falls back to the classic 80×25 on a small
screen. It opens on a live **WAR-DIAL** recon screen with braille animations, a
sub-cell-smooth progress bar, and a **port-scan bar per target IP** — each cell
a coloured pip (█ green = open, █ cyan = banner, ▒ dim = closed, ░ = pending).
...
**R** go dark / reconnect · arrows/**jk** move · **?** help · **Q** quit. In a
breach: pick a vector and a tool, **ENTER** to attack and drop the target's
SHIELD without maxing its ALARM; **G** to guess the password (looted intel ⚷
reveals it).

It's a toy, but the moves are real, and it tries to teach as you play: the
breach screen explains what each tool actually is (dictionary attack, default
creds, web exploit, buffer overflow, phishing, 0-day), and `?` maps every
mechanic to its real-world idea — the TRACE is an IDS/SOC, honeypots are decoy
hosts, looting is credential reuse, pivoting is lateral movement. While you're
live the **wire is alive**: insider leaks, blue-team hardening, maintenance
reboots, dropped exploits, and packet storms fire at random — each a surprise
*and* a one-line lesson. (There may also be a familiar greeting waiting on the
MAINFRAME.)

```sh
./darkcidr --game                       # LIVE scan of your local nets -> the world
...
it drops a shield, so pivoting through routers pays off. The end screen ranks
your run (LAMER → SCRIPT KIDDIE → PHREAK → ICE BREAKER → GHOST IN THE WIRE).
Every node, service, and marker has an icon; press `?` for the full **legend**.

**The game uses your actual network.** In the terminal, `--game` opens on a live
**RECON** screen: it scans a target (or your local networks) and the services
stream in as they're discovered — you **ENTER to jack in** once there's enough to
play (or it auto-starts when the scan finishes). The world is then built from what
it actually found: real IPs, open ports, and banners, with a few hosts flagged as
honeypots so the trap mechanic still bites. The recon is best-effort and capped at
a time budget; it uses a privilege-free TCP connect sweep by default (`--zmap`
opts into the raw-socket SYN scanner, which needs root). Pass **`--sim`** to skip
the scan and play the embedded sample world (the game also falls back to it if no
network is detected).
The browser build is always the sample world — a server has no business scanning
*your* LAN.

On a TTY, `--game` (no address) plays right in the CLI. Give it a listen address
(or run it headless) and the very same terminal game is streamed to a browser
through `ghostty-web` — identical world, identical rules, identical code — inside
the full retro CRT shell (matrix rain, power-on animation, scanlines, bezel). The
browser build always uses the sample world (the server can't scan *your*
network); the terminal high score lives in a small file under your config dir.

### The seeded world

The game is **seeded by real-zmap-format data**: `game/seed.js` (embedded in the
binary) is produced by the `toneloc-seed` tool, which walks the address space
with zmap-go's cyclic iterator (the same permutation the real scanner uses) — or
...
./darkcidr-seed -mask 10.37.0.0/16 > game/seed.js
...
The same seed always builds the same network (the mulberry32 RNG and FNV-1a hash
are matched to the generator), so a given world plays identically in the terminal
and the browser.
...
internal/engine        scan engine: IP masks, zmap-go iterator, response
                       ...
                       live stats, the network Map grid, and .DAT persistence
internal/dos           an 80x25, 16-colour MS-DOS text-mode renderer
                       (CP437 box-drawing, blink, a diffing ANSI flusher, SVG)
internal/tui           the 3-window dialer + the network Map view, keyboard and
                       mouse handling (arrow/SGR-mouse escape parsing)
internal/game          DARKCIDR, the war-dialing roguelike (the one and only
                       implementation; renders to internal/dos)
internal/web           HTTP + WebSocket bridge to ghostty.js, for both the
                       scanner UI and DARKCIDR
cmd/toneloc-seed       builds game/seed.js from zmap-go's iterator or a zmap CSV
game/                  seed.js: the embedded DARKCIDR sample world seed
```
...
*DARKCIDR rocks, and if you don't read the docs, you're a LAMER.*
